...
function createShader(){
    ...
    for(i=0; i<= shaderSelectors.length-1; i++){
        ...
        let shaderNumber;
        //si le module shader a déjà été crée, on va chercher le shaderNumber dans l'elementList correspondant
        if(shaderModuleNumber < elementList.length){
            shaderNumber = elementList[shaderModuleNumber].shader.length;
        }
        else{
            shaderNumber = 1;
        }
...
function createBox(){
    for(i=0; i<= boxSelectors.length-1; i++){
        ...
        let boxNumber;
        //si le module box a déjà été crée, on va chercher le boxNumber dans l'elementList correspondant
        if(boxModuleNumber < elementList.length){
            boxNumber = elementList[boxModuleNumber].box.length;
        }
        else{
            boxNumber = 1;
        }
        ...
        let boxSelectNumber;
        //si le boxNumber n'est pas égale a 1 on regarde l'option séléctionné, on prend sa valeur et l'attribur a boxSelelctNumber
        if(boxNumber == 1){
            boxSelectNumber = 1;
            console.log("test1");
        }
        else{
            let boxSelected = boxSelectors[boxModuleNumber].children;
